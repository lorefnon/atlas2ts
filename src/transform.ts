...
export const transformAtlasConfig = async (inputs: DBSchema[], config: TransformConfig) => {
    let output = generateOutput(inputs, config).join('\n');
    if (config.template) {
        const engine = new Liquid({
            root: config.templateRoot ? path.resolve(config.templateRoot) : process.cwd(),
            extname: '.liquid'
        });
...
export const NamingStrategySpec = z.literal('unmodified').or(z.literal('camel-case'));

export type NamingStrategy = z.TypeOf<typeof NamingStrategySpec>

export const GeneratorSpec = z.union([
    z.literal("ts"),
    z.literal("zod")
]);
...
const parseInputs = async (config: TransformConfig) =>
    flatten(await Promise.all(config.inputPaths.map(async (inputPath) => {
